...
class TestEbookOnlyNormalization:
    def test_returns_char_offsets_via_text_matching(self):
        ...
        mgr = _make_manager(
            sync_clients={"KoSync": client_a, "Grimmory": client_b},
            ebook_parser=parser,
        )

        config = {
            "KoSync": _make_state(0.30),
            "Grimmory": _make_state(0.50),
        }
        result = mgr._normalize_for_cross_format_comparison(_make_book(), config)

        assert result == {"KoSync": 30_000, "Grimmory": 50_000}
    ...
    def test_returns_none_without_ebook_filename(self):
        ...
        mgr = _make_manager(sync_clients={"KoSync": MagicMock(), "Grimmory": MagicMock()})
        config = {
            "KoSync": _make_state(0.30),
            "Grimmory": _make_state(0.50),
        }
        result = mgr._normalize_for_cross_format_comparison(_make_book(ebook_filename=None), config)
        assert result is None
...
class TestDetermineLeaderEbookOnly:
    def test_leader_chosen_by_char_offset_not_raw_pct(self):
        ...
        parser.resolve_book_path.return_value = "/books/book.epub"
        parser.extract_text_and_map.return_value = (full_text, [])
        # KoSync's text resolves to char 60000 (ahead), Grimmory to char 40000
        parser.find_text_location.side_effect = [
        ...
        mgr = _make_manager(
            sync_clients={"KoSync": client_ko, "Grimmory": client_bl},
            ebook_parser=parser,
        )

        # Grimmory has higher raw pct (0.50 vs 0.45), but KoSync is
        ...
        with patch.object(mgr, "_has_significant_delta", return_value=True):
            leader, leader_pct = mgr._determine_leader(config, _make_book(), "test-123", "Test Book")
