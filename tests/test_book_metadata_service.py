...
def _make_container(hc_configured=False, bl_configured=False):
    ...
    container.grimmory_client_group.return_value = bl_client
...
# ---------------------------------------------------------------------------
# Grimmory API failure
# ---------------------------------------------------------------------------


class TestGrimmoryFailure:
    """When Grimmory client raises, the metadata build should still succeed."""

    def test_grimmory_exception_does_not_crash(self):
        ...
        bl_client.find_book_by_filename.side_effect = ConnectionError("Grimmory down")

        result = build_book_metadata(book, Mock(), db, abs_service, grimmory_client=bl_client)

        assert "grimmory_url" not in result

    def test_grimmory_not_configured_skips_lookup(self):
        book = _make_book(ebook_filename="test.epub")
        abs_service = _make_abs_service(item_details=None)
        db = _make_db_service()
...
class TestCachingBehavior:
    ...
    def test_no_ebook_filename_skips_grimmory(self):
        ...
        build_book_metadata(book, container, db, abs_service, grimmory_client=bl_client)
