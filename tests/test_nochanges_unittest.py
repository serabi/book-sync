    ...
    def get_progress_mock_returns(self):
        ...
        return {
            "abs_progress": {
                "ebookProgress": 0.25,
                "currentTime": 250.0,
                "ebookLocation": "epubcfi(/6/2[chapter1]!/4/2[content]/4/1:0)",
            },  # 25%
            ...
            "grimmory_progress": (0.25, None),  # 25%
        }

    def test_no_changes_detected(self):
        """Test sync_cycle when no changes are detected (all deltas are zero)."""
        log_output = super().run_test(None, None)
        self.assertNotIn("States saved to database", log_output, "Logs should show no change")
