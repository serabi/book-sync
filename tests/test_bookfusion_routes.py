...
def test_grimmory_books_returns_supported_formats(client, mock_container):
    mock_container.mock_grimmory_client.is_configured.return_value = True
    mock_container.mock_grimmory_client.get_all_books.return_value = [
    ...
    resp = client.get("/api/bookfusion/grimmory-books")
...
def test_grimmory_books_with_search_query(client, mock_container):
    mock_container.mock_grimmory_client.is_configured.return_value = True
    mock_container.mock_grimmory_client.search_books.return_value = [
        {"id": 5, "title": "Searched", "authors": "A", "fileName": "searched.epub"},
    ]
    resp = client.get("/api/bookfusion/grimmory-books?q=searched")
    assert resp.status_code == 200
    data = resp.get_json()
    assert len(data) == 1
    mock_container.mock_grimmory_client.search_books.assert_called_once_with("searched")


def test_grimmory_books_not_configured(client, mock_container):
...
def test_grimmory_books_exception_returns_empty(client, mock_container):
    mock_container.mock_grimmory_client.is_configured.return_value = True
    mock_container.mock_grimmory_client.get_all_books.side_effect = Exception("Grimmory down")
    resp = client.get("/api/bookfusion/grimmory-books")
    assert resp.status_code == 200
    assert resp.get_json() == []
...
def test_upload_book_success(client, mock_container):
    ...
    mock_container.mock_grimmory_client.is_configured.return_value = True
    mock_container.mock_grimmory_client.download_book.return_value = b"file-bytes"
...
def test_upload_book_grimmory_not_configured(client, mock_container):
    mock_container.mock_bookfusion_client.upload_api_key = "key"
    mock_container.mock_grimmory_client.is_configured.return_value = False
    resp = client.post("/api/bookfusion/upload", json={"book_id": 1})
    assert resp.status_code == 400
    assert "Grimmory not configured" in resp.get_json()["error"]


def test_upload_book_download_fails(client, mock_container):
    mock_container.mock_bookfusion_client.upload_api_key = "key"
    mock_container.mock_grimmory_client.is_configured.return_value = True
    mock_container.mock_grimmory_client.download_book.return_value = None
    resp = client.post("/api/bookfusion/upload", json={"book_id": 1})
...
def test_upload_book_upload_fails(client, mock_container):
    mock_container.mock_bookfusion_client.upload_api_key = "key"
    mock_container.mock_grimmory_client.is_configured.return_value = True
    mock_container.mock_grimmory_client.download_book.return_value = b"bytes"
...
def test_add_to_dashboard_success(client, mock_container):
    ...
    resp = client.post(
        "/api/bookfusion/add-to-dashboard",
        json={
...
def test_match_to_book_link(client, mock_container):
    ...
    resp = client.post(
        "/api/bookfusion/match-to-book",
        json={
            "bookfusion_ids": ["bf-1", "bf-2"],
            "abs_id": "dash-1",
        },
    )
    assert resp.status_code == 200
