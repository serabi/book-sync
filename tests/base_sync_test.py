...
class BaseSyncCycleTestCase(unittest.TestCase, ABC):
    ...
    def setUp(self):
        ...
        transcript_data = [
            ...
            {
                "start": self.expected_final_pct * 1000,
                "end": self.expected_final_pct * 1000 + 10,
    ...
    def setup_common_mocks(self):
        ...
        grimmory_client._cache_timestamp = 0
        ...
        grimmory_client.is_configured.return_value = True
        ...
        grimmory_client.get_progress.return_value = progress_returns["grimmory_progress"]
        ...
        grimmory_client.update_progress.return_value = True
    ...
    def run_sync_test_with_leader_verification(self):
        ...
        transcriber = Mock()
        transcriber.get_text_at_time.return_value = (
            f"Sample text from {self.expected_leader} leader at {self.expected_final_pct * 100:.0f}%"
        )

        def find_time_for_text_side_effect(
            transcript_path, search_text, hint_percentage=None, book_title=None, **kwargs
        ):
        ...
        grimmory_sync_client = GrimmorySyncClient(mocks["grimmory_client"], mocks["ebook_parser"])

        # Create SyncManager with dependency injection (all mocks)
        manager = SyncManager(
            abs_client=mocks["abs_client"],
            grimmory_client=mocks["grimmory_client"],
    ...
    def verify_common_assertions(self, mocks, manager):
        ...
        if self.test_book.storyteller_uuid:
            self.assertTrue(
                mocks["storyteller_client"].get_position_details.called,
                "Storyteller get_position_details was not called",
            )
        self.assertTrue(mocks["grimmory_client"].get_progress.called, "Grimmory get_progress was not called")
        ...
        if leader != "NONE":
            ...
            self.assertTrue(
                mocks["ebook_parser"].find_text_location.called, "EbookParser find_text_location was not called"
            )
            ...
            if leader != "ABS":
                ...
                abs_updated = (
                    ...
                    or manager.sync_clients["ABS"]._update_abs_progress_with_offset.called
                )
                self.assertTrue(abs_updated, "ABS update was not called")
            if leader != "KOSYNC":
                # KoSync now accepts percentage-only updates (empty xpath) when
                # the original xpath is malformed and regeneration fails.
            ...
            if leader != "STORYTELLER":
                ...
                if self.test_book.storyteller_uuid:
                    self.assertTrue(
                        mocks["storyteller_client"].update_position.called, "Storyteller update_position was not called"
                    )
            if leader != "GRIMMORY":
                self.assertTrue(
                    mocks["grimmory_client"].update_progress.called, "Grimmory update_progress was not called"
                )
        ...
        mocks["grimmory_client"].get_progress.assert_called_with(ebook_file)
    ...
    def verify_final_state(self, manager):
        ...
        grimmory_pct = final_states.get("grimmory", 0)
        ...
        if self.get_expected_leader() != "None":
            ...
            self.assertAlmostEqual(
                ...
                expected_pct,
                delta=tolerance,
                msg=f"Grimmory final state {grimmory_pct:.1%} != expected {expected_pct:.1%}",
            )
    ...
    def run_test(self, from_percentage: float | None, target_percentage: float | None):
        ...
        try:
            ...
            if from_percentage is not None and target_percentage is not None:
                ...
                self.assertIn(
                    f"{self.get_expected_leader()} leads at {target_percentage}.0000%",
                    log_output,
                ...
                self.assertIn(
                    f"{self.get_expected_leader()}: {from_percentage}.0000% -> {target_percentage}.0000%",
                    log_output,
