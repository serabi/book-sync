"""Tests for Grimmory cover proxy endpoint and auth contract."""
...
from src.blueprints.covers import _proxy_grimmory_cover_for  # noqa: E402


class TestGrimmoryCoverProxy(unittest.TestCase):
    """Verify _proxy_grimmory_cover_for sends correct URL, auth, and headers."""
    ...
    def _make_client(self, configured=True, token="fake-jwt-token"):
        ...
        client.base_url = "http://grimmory.local"
    ...
    def test_uses_media_endpoint_path(self, mock_get, mock_dir):
        ...
        _proxy_grimmory_cover_for(bl, 3880)

        called_url = mock_get.call_args[0][0]
        self.assertEqual(called_url, "http://grimmory.local/api/v1/media/book/3880/cover")
    ...
    def test_auth_via_query_param_not_header(self, mock_get, mock_dir):
        ...
        _proxy_grimmory_cover_for(bl, 42)

        kwargs = mock_get.call_args[1]
        self.assertEqual(kwargs["params"], {"token": "my-secret-jwt"})
        self.assertNotIn(
            "headers", kwargs, "Should not send Authorization header — Grimmory media uses query-param auth"
        )
    ...
    def test_content_type_hardcoded_jpeg(self, mock_get, mock_dir):
        ...
        resp = _proxy_grimmory_cover_for(bl, 1)
    ...
    def test_successful_fetch_writes_cache_file(self, mock_get, mock_dir):
        ...
        _proxy_grimmory_cover_for(bl, 42, cache_prefix="bl")
    ...
    def test_serves_from_cache_when_unconfigured(self, mock_dir, mock_send):
        """When Grimmory is not configured but cache exists, serve cached cover."""
        mock_dir.return_value = self.covers_dir
        ...
        _proxy_grimmory_cover_for(bl, 7)
    ...
    def test_not_configured_no_cache_returns_404(self, mock_dir):
        mock_dir.return_value = self.covers_dir
        bl = self._make_client(configured=False)
        result = _proxy_grimmory_cover_for(bl, 1)
    ...
    def test_upstream_404_no_cache_returns_404(self, mock_get, mock_dir):
        ...
        mock_get.return_value = Mock(status_code=404)
        bl = self._make_client()
        result = _proxy_grimmory_cover_for(bl, 9999)
        self.assertEqual(result, ("Cover not found", 404))
    ...
    def test_network_error_with_cache_serves_cached(self, mock_get, mock_dir, mock_send):
        ...
        _proxy_grimmory_cover_for(bl, 5)
