                ...
                if line.startswith("[") and "] " in line:
                    ...
                    if ": " in rest:
                        ...
                        if level_num >= min_level_num:
                            if (
                                not search_term
                                or search_term in message.lower()
                                or search_term in level_str.lower()
                                or search_term in module_str.lower()
                            ):
                                parsed_logs.append(
                                    {
                    ...
                    if min_level_num <= 20:
                        if not search_term or search_term in line.lower():
                            parsed_logs.append(
                                {"timestamp": "", "level": "INFO", "message": line, "module": "unknown", "raw": line}
                            )
    ...
    try:
        ...
        for log_entry in recent_logs:
            ...
            if level_num >= min_level_num:
                if (
                    not search_term
                    or search_term in log_entry["message"].lower()
                    or search_term in log_entry["level"].lower()
                ):
                    filtered_logs.append(log_entry)

        result_logs = filtered_logs[-count:] if len(filtered_logs) > count else filtered_logs

        return jsonify({"logs": result_logs, "timestamp": datetime.now().isoformat()})
