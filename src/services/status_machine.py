...
class StatusMachine:
    ...
    - 'local': journal + HC push + Booklore push + TBR cleanup + date fill
    ...
    def transition(self, book, new_status, source, *, container=None,
                   dates=None, allowed_from=None):
        ...
        Args:
            ...
            container: DI container (needed for HC push, Booklore push, date pull).
        ...
        # Booklore push (local only)
        if source == 'local' and book.ebook_filename and container:
            self._push_to_booklore(book, new_status, old_status, container)
    ...
    def _push_to_booklore(self, book, new_status, old_status, container):
        """Push read status to Booklore for relevant transitions."""
